...
from webdataset import DataPipeline, ResampledShards
import ctypes
import inspect
import threading
...
def rectangle_crop(arr, image_size, reshape_mode="center"):
    ...
    arr = TT.functional.crop(arr, top=top, left=left, height=new_h, width=new_w)
    return arr


def load_video_with_pose(
    video_data,
    pose_data,
    ori_height,
    ori_width,
    ...
    assert len(vr) == len(vr_pose), (
        f"frames and pose frames should have the same length, but here has len(vr): {len(vr)} and len(vr_pose): {len(vr_pose)}"
    )
    ...
    tensor_frms_pose = (
        torch.from_numpy(temp_frms_pose)
        if type(temp_frms_pose) is not torch.Tensor
        else temp_frms_pose
    )
    tensor_ref_frms = (
        torch.from_numpy(ref_frms) if type(ref_frms) is not torch.Tensor else ref_frms
    )
    tensor_ref_frms_pose = (
        torch.from_numpy(ref_frms_pose)
        if type(ref_frms_pose) is not torch.Tensor
        else ref_frms_pose
    )
    ...
    tensor_frms_pose = rectangle_crop(
        tensor_frms_pose, image_size, reshape_mode="center"
    )
    tensor_ref_frms = rectangle_crop(tensor_ref_frms, image_size, reshape_mode="center")
    tensor_ref_frms_pose = rectangle_crop(
        tensor_ref_frms_pose, image_size, reshape_mode="center"
    )
...
def load_video_with_pose_with_timeout(*args, **kwargs):
    ...
    return (
        video_container.get("video", None).contiguous(),
        video_container.get("pose", None).contiguous(),
        video_container.get("ref", None).contiguous(),
        video_container.get("ref_pose", None).contiguous(),
    )
