...
def load_image_to_tensor_chw_normalized(image_data):
    ...
    image = Image.open(image_data).convert(
        "RGB"
    )  # Convert to RGB in case it's a grayscale image or has an alpha channel
...
def read_from_cli():
    ...
    try:
        while True:
            x = input(
                "Please input in format like <prompt>@@<example_dir>, e.g. the girl is dancing@@examples/001 (Ctrl-D quit): "
            )
            yield x.strip(), cnt
            cnt += 1
...
def sampling_main(args, model_cls):
    ...
    if args.input_type == "cli":
        assert mpu.get_data_parallel_world_size() == 1, (
            "Only dp = 1 supported in cli mode."
        )
        ...
        data_iter = read_from_file(
            args.input_file, rank=dp_rank, world_size=dp_world_size
        )
    else:
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            if args.image2video:  # i2v 输入是图片+prompt
                if args.use_pose:
                    ...
                    if image_path is None:
                        raise FileNotFoundError(
                            f"Reference image not found in {input_dir}. Tried: {ref_image_patterns}"
                        )
                    ...
                    if pose_path is None:
                        raise FileNotFoundError(
                            f"Pose video not found in {input_dir}. Tried: {pose_patterns}"
                        )
                ...
                pose_video = resize_for_rectangle_crop(
                    pose_video, [target_H, target_W], reshape_mode="center"
                )
                pose_video = (pose_video - 127.5) / 127.5  # color value: 0-255 -> -1-1
                sampling_num_frames = pose_video.shape[0]
                if sampling_num_frames > 81:
                    raise ValueError(
                        "Too many frames in pose video, please provide <= 81 frames."
                    )
                ...
                if GT is not None:
                    GT = resize_for_rectangle_crop(
                        GT, [target_H, target_W], reshape_mode="center"
                    )
                ...
                if model.i2v_encode_video:  # wan的模式,不需要再重复或者替换第一帧
                    assert args.use_pose, (
                        "wan for not using pose has not been merged into this version"
                    )
                    pose_video = (
                        pose_video.unsqueeze(0).to("cuda").to(torch.bfloat16)
                    )  # B T C H W
                    if "smpl" in args.representation:
                        smpl_render_video = (
                            smpl_render_video.unsqueeze(0).to("cuda").to(torch.bfloat16)
                        )  # B T C H W
                    ori_image = (
                        image_tensor.unsqueeze(0).to("cuda").to(torch.bfloat16)
                    )  # B 1 C H W, -1-1
                    image_to_save = ori_image.repeat(1, pose_video.shape[1], 1, 1, 1)
                    image = torch.concat(
                        [ori_image, torch.zeros_like(pose_video[:, 1:])], dim=1
                    )
                    ...
                    ref_concat = model.encode_first_stage(
                    ...
                    if model.noised_image_all_concat:
                        ...
                        image = torch.concat(
                            [
                                image,
                                torch.zeros(pad_shape).to(image.device).to(image.dtype),
                            ],
                            dim=1,
                        )

                if "smpl" in args.representation:
                    smpl_render_latent = model.encode_first_stage(
                        rearrange(
                            smpl_render_video, "b t c h w -> b c t h w"
                        ).contiguous(),
                        None,
                        force_encode=True,
                    )
                    smpl_render_latent = smpl_render_latent.permute(
                        0, 2, 1, 3, 4
                    ).contiguous()  # B, T, C, H, W
                    pose_latent = smpl_render_latent
                else:
                    pose_latent = model.encode_first_stage(
                        rearrange(pose_video, "b t c h w -> b c t h w").contiguous(),
                        None,
                        force_encode=True,
                    )
                    pose_latent = pose_latent.permute(
                        0, 2, 1, 3, 4
                    ).contiguous()  # B, T, C, H, W
                ...
                if model.use_i2v_clip:
                    ...
                    image_clip_features = model.i2v_clip.visual(
            ...
            for index in range(args.batch_size):
                ...
                if mpu.get_sequence_parallel_rank() == 0:
                    ...
                    if args.only_save_latents:
                        ...
                        samples_x = model.decode_first_stage(samples_z).to(
                            torch.float32
                        )
                        ...
                        if mpu.get_model_parallel_rank() == 0:
                            if GT is not None:
                                if "smpl_downsample" in args.representation:
                                    smpl_render_video = F.interpolate(
                                        smpl_render_video.squeeze(0),
                                        size=(GT.shape[2], GT.shape[3]),
                                        mode="bilinear",
                                        align_corners=False,
                                    ).unsqueeze(0)  # t c h w -> 1 t c h w
                                    save_list = [
                                        torch.clamp(
                                            (smpl_render_video + 1.0) / 2.0,
                                            min=0.0,
                                            max=1.0,
                                        ).cpu(),
                                        torch.clamp(
                                            (image_to_save + 1.0) / 2.0,
                                            min=0.0,
                                            max=1.0,
                                        ).cpu(),
                                        torch.clamp(
                                            (GT.unsqueeze(0) + 1.0) / 2.0,
                                            min=0.0,
                                            max=1.0,
                                        ).cpu(),
                                    ]
                                ...
                                save_multi_video_grid_and_mp4(
                                    save_list + [samples],
                                    save_dir,
                                    fps=args.sampling_fps,
                                    key=f"{test_case_idx}_concat",
                                )  # 都要求是B T C H W
                            save_multi_video_grid_and_mp4(
                                [samples],
                                save_dir,
                                fps=args.sampling_fps,
                                key=f"{test_case_idx}_output",
                            )
...
if __name__ == "__main__":
    ...
    if (
        "sigma_sampler_config" in args.model_config.loss_fn_config.params.keys()
        and hasattr(
            args.model_config.loss_fn_config.params.sigma_sampler_config.params,
            "uniform_sampling",
        )
    ):
    ...
    elif args.model_type == "cross_causal":
        Engine = diffusion_video_causal.CausalVideoEngine
    else:
        Engine = diffusion_video_tasd.TASDEngine
